...
# 回调代理超时（秒）：回调可能是慢调用（如 LLM 推理 + 重试退避），
# 超时过短会导致真实客户端拿 502、回调侧写响应报 BrokenPipeError
...
class DaemonHandler(BaseHTTPRequestHandler):
    ...
    def _proxy_to_callback(self, callback_url, method, path):
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=_CALLBACK_TIMEOUT) as resp:
                resp_body = resp.read()
                resp_ct = resp.headers.get("Content-Type", "application/json")
                self._send_proxied(resp.status, resp_ct, resp_body)
            ...
            self._send_proxied(e.code, resp_ct, resp_body)
        except Exception as e:
            _log(f"Callback proxy error for {method} {path}: {e}", "ERROR")
            self._send_json(502, {"error": f"Callback server error: {e}"})

    def _send_proxied(self, status_code, content_type, body_bytes):
        """将回调响应转发给真实客户端，客户端已断连时静默。"""
        try:
